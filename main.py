...
class Human:
    ...
    def indexes_day(self, day):
        day = f'Today is a {day} of {self.name}`s life'
        print(f'{day:=^50}')
        ...
        print(f'Fuel = {self.car.fuel:^50}')
        print(f'Strenght = {self.car.strength:^50}')
def is_alive(self):
    if self.gladness <=0:
            print('Depression')
            return False
...
def live(self, day):
    ...
    if self.job is None:
        self.get_job()
        print(f"I don't want a job, going to get a job {self.job.job}"
                f"with salary {self.job.salary}")
        self.indexes_day(day)
        dice = random.randint(1,4)
    if self.satiety < 10:
            print(f'I am going to eat')
            self.eat()
